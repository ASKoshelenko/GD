...
from core.python.utils import (
    ...
    disable_protection,
    clearScenario3UserResources,
    getUserFromDbScenario3,
    removeKeyPairs,
    terminateInstances,
    formatTags,
    getUserIdListFromCollection,
    processInstanceTags,
    clearScenario6UserResources,
    clearScenario8UserResources,
    getUserFromDbScenario6,
    getCreatedResources,
    getResourceIds,
    deleteEC2SecurityGroups,
    deleteDBSnapshots,
    deleteRDSInstances,
    processTargets,
    createS3remote,
    deleteS3remote,
    clearScenario7UserLambda,
    detachUserPolicy
)
...
class CloudGoat:
    def __init__(self, base_dir):
        ...
        self.scenario_names = dirs_at_location(self.scenarios_dir,
                                               names_only=True)
        self.whitelist_path = os.path.join(base_dir, "whitelist.txt")

        self.aws_region = "eu-central-1"
        self.cloudgoat_commands = ["config", "create", "destroy", "list",
                                   "help"]
        ...
        self.backend_bucket = "aws-game-day-tfstate-bucket"

    def parse_and_execute_command(self, parsed_args):
        command = parsed_args.command
        profile = parsed_args.profile
        smtppassword = parsed_args.smtppassword
        ...
        if command[0] == "config":
            ...
            if command[1] == "init":
                return self.create_scenario(command[1], profile, smtppassword, user)
            else:
            ...
            if command[1] == "all":
                ...
                return self.destroy_scenario(command[1], profile, smtppassword, username=user)
    ...
    def configure_or_check_whitelist(self, auto=False, print_values=False):
        if auto:
            message = (
                f"AWS SGD can automatically make a network request, using "
            ...
            if confirm_auto_configure.strip().lower().startswith("y"):
                ...
                if ip_address is None:
                    print(
                        f"\n[AWS SGD] Unknown error: Unable to retrieve IP address.\n")
                    return None
    ...
    def create_scenario(self, scenario_name_or_path, profile, smtppassword, username=""):
        if username:
        ...
        if scenario_name == "init":
            createS3remote(profile,self.aws_region, self.backend_bucket)

        init_retcode, init_stdout, init_stderr = terraform.init(
           capture_output=False, no_color=IsNotFlagged, backend_config=[("profile=" + profile), ("region=" + self.aws_region), ("bucket=" + self.backend_bucket)]
        )
        if init_retcode != 0:
            display_terraform_step_error(
        ...
        if scenario_name == "init":
            terraform_variables["smtpPassword"] = smtppassword

        plan_retcode, plan_stdout, plan_stderr = terraform.plan(
            capture_output=False,
            var=terraform_variables,
    ...
    def destroy_scenario(self, scenario_name_or_path, profile, smtppassword, confirmed=False, username=""):
        # Information gathering.
        ...
        print(scenario_instance_dir_path)
        scenario_dir = os.path.join(self.scenarios_dir, scenario_name)
        if not scenario_dir or not scenario_name:
            if not scenario_name:
                return print(
                    f"No recognized scenario name was entered. Did you mean one of"
                    f" these?\n    " + f"\n    ".join(self.scenario_names)
            ...
            if not scenario_dir:
                return print(
                    f"No dir with scenario name was found. something is broken"
                )
        if not scenario_instance_dir_path:
            # prepare scenario folder if not exists
            print("attempt to destroy remote scenario")
            cgid = generate_cgid()
            instance_path = os.path.join(
                self.base_dir, f"{scenario_name}_{cgid}"
            )
        ...
        # Clear resources for scenario3 maded by users
        if scenario_name == "scenario3":
            disable_protection(profile, scenario_name)
            clearScenario3UserResources(profile)

        # Clear resources for scenario6 maded by users        
        if scenario_name == "scenario6":
            clearScenario6UserResources(profile)

        # Clear resources for scenario7 maded by users
        if scenario_name == "scenario7":
            clearScenario7UserLambda(profile)
            detachUserPolicy(profile)
 
        # Clear resources for scenario8 maded by users
        if scenario_name == "scenario8":
            clearScenario8UserResources(profile)
        ...
        # Set smtp Password variable if it init
        if scenario_name == "init":
            terraform_variables["smtpPassword"] = "themostsecure"
        ...
        if destroy_retcode != 0:
            display_terraform_step_error(
                "terraform destroy", destroy_retcode, destroy_stdout,
                destroy_stderr
            )
            return
        else:
            print("\n[cloudgoat] terraform destroy completed with no error code.")
            if scenario_name == "init":
                print("[cloudgoat] destroying backend")
                if deleteS3remote(profile):
                    print("[cloudgoat] s3 backend destroyed")
                else: 
                    print("[cloudgoat] ERROR !! s3 backend was not destroyed")
