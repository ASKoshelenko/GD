...
class Terraform(object):
    ...
    def version(self):
        ...
        try:
            ver = \
                [l for l in out.splitlines() if 'Terraform' in l][0].split(' ')[
                    -1]
            return float('.'.join(ver.strip('v').split('.')[0:-1]))
        except KeyError:
            raise UserWarning(
                'Could not find version in `terraform version` output')

    def apply(self, dir_or_plan=None, input=False, skip_plan=False,
              no_color=IsFlagged,
              **kwargs):
        """
